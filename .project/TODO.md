...
- [x] **QA-08** — Extend `e2e/theme-toggle.spec.js`: toggle behavior + persistence
  ...
  - Cross-page persistence: set `localStorage.theme = "light"`, navigate `/` → `/projects` → `/contact`, assert `data-theme="light"` holds on each
  - Simulated `localStorage` failure (e.g. via `page.addInitScript` overriding `localStorage.setItem`/`getItem` to throw): page still renders, footer year + `.reveal` classes + (on `/contact`) the submit handler still work, no uncaught page error event
...
- [x] **DEV-06** — Frontend Engineer: toggle markup + theme-init/persistence JS across all 3 pages
  ...
  - Depends On: QA-07, QA-08, QA-09, DEV-05 (needs the CSS variables to exist to have any visible effect)
...
Tags tasks to `.project/milestones.md` M5 (OA-17 cross-project DNS handoff for `picks.ericreilly.com`) so `~/.claude/fleet/scripts/milestone-rollup.js` can compute its completion. See `.project/milestones.md` for the full AC list this milestone is scored against.
