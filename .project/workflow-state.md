...
**Item 2 — IN PROGRESS.** Dispatching DevOps to add a `paths:`/`paths-ignore:` filter to `deploy.yml`'s `push: branches: [main]` trigger so docs-only/test-only commits stop re-triggering staging deploys (fired twice on 2026-09-22 closeout pushes — see CLD-13 Stage 4 note above). QA to verify with a docs-only commit (no deploy run) and an app-code commit (deploy runs).
